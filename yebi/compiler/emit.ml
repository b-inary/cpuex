...
and g' oc = function
  ...
  | (NonTail _, Nop) -> ()
  | (NonTail x, Li i)          -> Printf.fprintf oc "    set     %s, %d\n" x i
  | (NonTail x, MovL (Id.L y)) -> Printf.fprintf oc "    set     %s, %s\n" x y
  ...
  | (NonTail x, Ld (y, z))     -> Printf.fprintf oc "    load    %s, [%s + %d]\n" x y z
  | (NonTail _, St (x, y, z))  -> Printf.fprintf oc "    store   %s, [%s + %d]\n" x y z
  ...
  | (NonTail _, Push (x, y)) when List.mem x allregs && not (S.mem y !stackset) ->
      save y; Printf.fprintf oc "    store   %s, [%s + %d]\n" x reg_sp (offset y)
